...
const convert_inform = function(str) {
  return str.replaceAll("_", " ").trim();
}
const convert_plot = function(str) {
  return str.slice(0, 4).toUpperCase();
}
const convert_time = (str) => str.replace(":", "h");


const display_informed_flight = function(){
  const arr_message = flights.split('+');
  // console.log(tmp);
  let ans = [];
  for(const message of arr_message) {
    // convert each message to formal text

    let [inform, start, end, time] = message.split(";");
    ans.push([convert_inform(inform), convert_plot(start), convert_plot(end), convert_time(time)]);
    
  }
  for (const e of ans) {
    // let tmp = ""

   const tmp =  `${e[0].includes("Delayed") ? "🔴 " : ""}${e[0]} from ${e[1]} to ${e[2]} (${e[3]})`;
    console.log(tmp.padStart(50, " "));
  }
}

display_informed_flight();
